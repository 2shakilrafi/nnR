test_that("a must be a number and nu a neural network and they must be in the
  ...
  c(5, 6) |>
    ...
    expect_error()
  5 |>
    srm(create_neural_network(c(4, 3, 2))) |>
    expect_error()
  create_neural_network(c(4, 3, 1)) |>
    slm(5) |>
    expect_error()

  c(5, 6) %slm% c(4, 5) |> expect_error()
  c(5, 6) %srm% c(4, 5) |> expect_error()
  5 %srm% create_neural_network(c(4, 3, 2)) |> expect_error()
  create_neural_network(c(4, 3, 1)) %slm% 5 |> expect_error()
